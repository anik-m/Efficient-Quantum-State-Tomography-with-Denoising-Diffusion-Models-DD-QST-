...
# -----------------------------------------------------------------------------
# Visualization Functions (Denoised Only)
# -----------------------------------------------------------------------------

def plot_denoised_only(target_rho, denoised_rho, fidelity, save_prefix="qst_denoised"):
    """
    Generates visualizations strictly for the Denoised (Model) output.
    """
    print(f"Generating Denoised Visualizations (Fidelity: {fidelity:.4f})...")
    
    # --- VISUAL 1: State City Plot (The "Product" Shot) ---
    # This shows the real part of the density matrix your model reconstructed
    fig = plot_state_city(denoised_rho, title=f"Denoised State Reconstruction\nFidelity: {fidelity:.4f}")
    fig.savefig(f"{save_prefix}_city_plot.png", dpi=300)
    plt.close(fig) # Close to free memory
    print(f"Saved: {save_prefix}_city_plot.png")

    # --- VISUAL 2: Error Heatmap (Target vs Denoised) ---
    # Shows exactly where your model's reconstruction differs from the ground truth
    diff_matrix = np.abs(target_rho.data - denoised_rho.data)
    
    plt.figure(figsize=(8, 6))
    sns.heatmap(diff_matrix, cmap="Reds", annot=False)
    plt.title(f"Reconstruction Error Magnitude\nAvg Abs Error: {np.mean(diff_matrix):.5f}")
    
    plt.tight_layout()
    plt.savefig(f"{save_prefix}_error_heatmap.png", dpi=300)
    plt.close()
    print(f"Saved: {save_prefix}_error_heatmap.png")
...
def main():
    ...
    # High shot count for smooth reconstruction
    shots_infer = 10000 
    ...
    # 6. Reconstruction
    print("--- Reconstructing Denoised State ---")
    ...
    print(f"\n=== RESULTS (Denoised Only) ===")
    print(f"Fidelity:             {fid_denoised:.5f}")
    print(f"Purity:               {purity:.5f}")
    print(f"Von Neumann Entropy:  {vn_ent:.5f}")
    
    # 8. Visualization (Safe Plotting)
    try:
        plot_denoised_only(target_dm, rho_denoised, fid_denoised)
